...
<div class="jumbotron">
	<div class="container">
		...
		<?php if (isset($_SESSION['id']) AND isset($_SESSION['mail']) && !($_SESSION['type'] == 'root'))
		{
		...
		if(!isset($_GET['type'])){
			...
			<?php
				...
				//Si les inscriptions sont fermées
			if(!getInfoSiteInformation("inscriptionsOuvertes") && !($_SESSION['type'] == 'root')){
				...
				<div id="inscription">
					...
					<form data-toggle="validator" role="form" class="form-horizontal" id="formPhaseI" action="creation_compte_entreprise.php" method="post" enctype="multipart/form-data">
						...
						<div class="form-group">
							...
							<div class="col-sm-8">
								<input type="password" placeholder="Minimum 8 caractères avec 1 chiffre" name="passEntreprise" id="passEntreprise" class="form-control"  pattern="(?=.*\d)(?=.*[a-zA-Z]).{8,}" data-minlength="8"  required/>
							</div>
						</div>
						<!-- Confirmation -->
						<div class="form-group">
